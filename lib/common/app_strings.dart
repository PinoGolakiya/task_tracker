class AppStrings {
  ///<<<<----------------- Splash screen strings ----------------->>>
  static const String splashImage =
      'https://img.freepik.com/free-psd/3d-icon-social-media-app_23-2150049601.jpg?w=826&t=st=1720415146~exp=1720415746~hmac=7031f6ea8b07756bd250ad53f383b8f95361dbca6e30a5d1521938bb970c967c';
  static const String appName = "Task Tracker";

  ///<<<<----------------- Settings screen strings ----------------->>>
  static const String settings = "Settings";
  static const String changeTheme = "App - Theme";

  ///<<<<----------------- Dashboard screen strings ----------------->>>
  static const String board = "Board";
  static const String noboard = "No boards found.";
  static const String toDo = "To-Do";
  static const String inProgress = "In-Progress";
  static const String done = "Done";

  ///<<<<----------------- Create Board screen strings ----------------->>>
  static const String createBoard = "Create Board";
  static const String createTask = "Create Task";
  static const String title = "Title";
  static const String create = "create";
  static const String boardName = "Board Name";
  static const String discription = "Discription";

  ///<<<<----------------- Board view screen strings ----------------->>>
  static const String taskBoardApp = "Task board app";
  static const String newString = "New";
  static const String pagination = "Pagination (infinite scroll)";

}
